...
/**
 * An {@link AsyncTask} that caches a list of tweets in a local file. This is
 * achieved by serialization of the List object containing the tweets.
 * 
 * <p>
 * To signify that the serialization task can take a non-trivial amount of time
 * given a large number of tweets, this AsyncTask introduces a delay of five
 * seconds before the tweets are written to file.
 * </p>
 */
public class AsyncWriteTweets extends AsyncTask<List<Tweet>, Void, Void> {
	...
	protected Void doInBackground(List<Tweet>... tweets) {
		...
		try {
			...
			Log.d(TAG,
					"File path = "
							+ test.getFileStreamPath(TWEETS_CACHE_FILE)
